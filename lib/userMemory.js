...
export async function updateSharedMemory(userId, updates) {
  ...
  console.log("BEFORE UPDATE:", dotUpdates);

await updateDoc(memoryRef(uid), dotUpdates);

console.log("AFTER UPDATE");

const verify = await getUserMemory(uid);

console.log(
  "VERIFY ACTIVE TASK:",
  JSON.stringify(verify.shared.activeTask, null, 2)
);
...
export function buildMemoryContext(memory, pillarId) {
  ...
  if (
  shared.activeTask?.pillar === pillarId &&
  shared.activeTask.status === "pending"
) {
  const title = shared.activeTask.title || "in progress";
  lines.push(`Active task: ${title}`);
  }
...
export async function recordAssistantResponse(userId, pillarId, structured) {
  console.log("[MEMORY] recordAssistantResponse called");

  if (!userId || !structured) {
    console.log("[MEMORY] Missing userId or structured");
    return;
  }

  console.log("[MEMORY] needs_more_info:", structured.needs_more_info);
  console.log("[MEMORY] task:", structured.task);
  ...
  if (
    ...
    console.log("[MEMORY] Saving active task...");
    ...
    console.log("[MEMORY] Active task saved.");
  }

  return null;
}
// Returns the user's active task only for the current pillar
export async function getActiveTask(userId, pillarId) {
  ...
  // Don't show a task belonging to another pillar
