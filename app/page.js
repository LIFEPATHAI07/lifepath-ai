...
const parseLinks = (text) => {
  const map = {
    ...
    "shine.com": "✨ Shine", "foundit.in": "🔍 Foundit",
    "timesjobs.com": "⏰ TimesJobs", "apna.co": "👔 Apna",
    "workindia.in": "💼 WorkIndia", "norkaroots": "🌍 NORKA Gulf",
    "keralapsc.gov.in": "🏛️ Kerala PSC", "startupmission": "🚀 KSUM",
    "startupindia.gov": "🇮🇳 Startup India", "angellist": "👼 AngelList",
    "letsventure": "💡 LetsVenture", "coursera.org": "📚 Coursera",
    "udemy.com": "🎯 Udemy", "nptel.ac.in": "🎓 NPTEL Free",
    "upwork.com": "💻 Upwork", "fiverr.com": "⚡ Fiverr",
    "groww.in": "📈 Groww", "zerodha": "💰 Zerodha",
    "scholarships.gov.in": "🏆 Scholarships", "gst.gov.in": "📋 GST",
    "udyamregistration": "🏭 MSME", "fssai.gov.in": "🍽️ FSSAI",
    "ipindia.gov.in": "™️ Trademark", "amazon": "🛒 Amazon",
    "meesho": "🛍️ Meesho", "promptbase.com": "🤖 PromptBase",
    "gumroad.com": "💸 Gumroad", "hostinger": "🌐 Hostinger",
    "canva.com": "🎨 Canva Free", "cee.kerala.gov.in": "📝 KEAM",
    "jeemain": "📝 JEE", "neet.nta": "📝 NEET",
    "scale.ai": "🤖 Scale AI", "toloka.ai": "🤖 Toloka",
    "printful.com": "👕 Printful", "etsy.com": "🛍️ Etsy",
    "neom.com": "🏗️ NEOM Jobs", "larsentoubro": "🏢 L&T",
    "kefholdings": "🏢 KEF", "cial.aero": "✈️ CIAL",
    "infosys.com": "💻 Infosys", "wipro.com": "💻 Wipro",
    "tcs.com": "💻 TCS", "tataelxsi": "💻 Tata Elxsi",
  };
  try {
...
const PILLARS = [
  ...
  { id: "cv", icon: "📄", label: "CV Builder", sub: "ATS Score · Build · Improve", color: "#06b6d4", rgb: "6,182,212", hint: "Build me an ATS-optimized CV for MEP Electrical Engineer jobs" },
  ...
  { id: "hustle", icon: "💸", label: "Side Hustle", sub: "Extra Income · A to Z Guide · Real", color: "#a855f7", rgb: "168,85,247", hint: "I have 2 free hours daily. Best side hustle with full guide?" },
...
const Circle = ({ score, size = 80, stroke = 6, color = "#06b6d4", label }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
      ...
      {label && <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textAlign: "center", maxWidth: 75 }}>{label}</div>}
    </div>
  );
};
      ...
      <SCard>
        ...
        {m.MONTHLY_SAVE_TARGET && (
          <div style={{ marginTop: 8, padding: "8px 10px", background: "rgba(16,185,129,0.08)", borderRadius: 9, border: "1px solid rgba(16,185,129,0.2)" }}>
            <span style={{ color: "#94a3b8", fontSize: 11 }}>Monthly Save Target: </span>
    ...
    <div>
      ...
      {m.BEST_HUSTLE && (
        <SCard>
          ...
          <div style={{ color: "#a855f7", fontSize: 14, fontWeight: 700, lineHeight: 1.4 }}>{m.BEST_HUSTLE}</div>
          {m.INITIAL_INVESTMENT && <div style={{ color: "#64748b", fontSize: 11, marginTop: 6 }}>Investment: <span style={{ color: "#e2e8f0", fontWeight: 600 }}>{m.INITIAL_INVESTMENT}</span></div>}
        </SCard>
      )}
    </div>
  );
  ...
  return (
    <div>
      {clean.split("\n").map((line, i) => {
        ...
        if (t.includes("⚠️") && (t.includes("guidance only") || t.includes("general guidance"))) return (
          <div key={i} style={{ marginTop: 14, padding: "10px 12px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.18)", borderRadius: 10, color: "#f59e0b", fontSize: 11, lineHeight: 1.6 }}>{t}</div>
        );
        if ((t.includes("🛡️") && (t.includes("watching") || t.includes("protection") || t.includes("career") || t.includes("safe"))) || (t.includes("💸🛡️") || t.includes("📄🛡️") || t.includes("🔍🛡️") || t.includes("💰🛡️") || t.includes("🚀🛡️"))) return (
          <div key={i} style={{ marginTop: 14, padding: "10px 12px", background: "rgba(6,182,212,0.07)", border: "1px solid rgba(6,182,212,0.18)", borderRadius: 10, color: "#06b6d4", fontSize: 12, fontStyle: "italic", fontWeight: 600 }}>{t}</div>
        );
        if (/^(ROLE|COMPANY|SALARY|APPLY|MATCH):/.test(t)) return null;
        if (t.startsWith("━")) return <div key={i} style={{ height: 1, background: "rgba(255,255,255,0.07)", margin: "12px 0" }} />;
        ...
        const isHeader = /^[🛡️📄🔍💰💸🚀🎯📊⚡✅❌💼💡🏢📋🌍👼🏆📅💪🎓📈🏦🚨⚖️🌐🎨👕🔥⭐💎🏗️]/.test(t) || /^#{1,3}\s/.test(t) || (t.endsWith(":") && t.length < 65 && !t.includes("http") && !t.includes("Rs") && !t.includes("Step"));
        if (isHeader) return (
          <div key={i} style={{ marginTop: 14, marginBottom: 4, fontWeight: 800, fontSize: 13, color: "#e2e8f0" }}>{t.replace(/^#{1,3}\s/, "")}</div>
        );
...
const ShareCard = ({ pillar, metrics, lastMsg, onClose }) => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      ...
      navigator.clipboard?.writeText(shareText).then(() => alert("Copied! Paste on WhatsApp or Instagram.")).catch(() => alert("Copy this text:\n\n" + shareText));
    }
  };
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 300, display: "flex", alignItems: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#0a1020", borderRadius: "22px 22px 0 0", padding: "24px 20px 44px", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ color: "#fff", fontWeight: 800, fontSize: 18, marginBottom: 6 }}>📤 Share Your Result</div>
        <div style={{ color: "#64748b", fontSize: 12, marginBottom: 18 }}>Help your friends discover LifePath AI!</div>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 13, padding: "14px", marginBottom: 16 }}>
        ...
        <button onClick={onClose} style={{ width: "100%", padding: 10, background: "transparent", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, color: "#64748b", fontSize: 13, cursor: "pointer" }}>Cancel</button>
      </div>
    </div>
  );
};
...
const ProfileModal = ({ profile, onSave, onClose }) => {
  ...
  const fields = [
    ...
    ["location", "City (e.g. Kochi, Malappuram)", "#ec4899"],
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#0a1020", borderRadius: "22px 22px 0 0", padding: "26px 20px 44px", border: "1px solid rgba(255,255,255,0.07)", maxHeight: "88vh", overflowY: "auto" }}>
        ...
        <div style={{ color: "#64748b", fontSize: 12, marginBottom: 22 }}>The more you share, the more personal every answer becomes.</div>
...
export default function LifePathAI() {
  ...
  const fireToast = (msg) => { setToast(msg); setTimeout(() => setToast(""), 3500); };
  ...
  const saveProfile = (p) => {
    setProfile(p); S.set("lp_profile", p); setShowProfile(false);
    fireToast("✅ Profile saved! Answers will now be personal to you.");
  };
  ...
  const handleFileUpload = (e) => {
    ...
    if (fileName.endsWith('.pdf')) {
      fireToast("⚠️ PDF cannot be read directly. Please copy-paste your CV text into the chat.");
      setInput("Here is my CV information:\nName: \nEducation: \nExperience: \nSkills: \nTarget Job: \n\nPlease analyze and improve it.");
      return;
    }
    ...
    reader.onload = (ev) => {
      ...
      if (!text || text.startsWith('%PDF') || text.length < 20) {
        fireToast("⚠️ Cannot read this file. Please paste your CV text directly in the chat.");
    ...
    reader.onerror = () => {
      fireToast("⚠️ Cannot read file. Please paste your CV text in the chat.");
    };
  ...
  const openShare = (msgContent) => {
    setShareMsg(msgContent);
    setShowShare(true);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060b14", fontFamily: "'Syne','Segoe UI',sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative", overflowX: "hidden" }}>
      ...
      {!accepted && screen !== "splash" && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.96)", zIndex: 500, display: "flex", alignItems: "flex-end" }}>
          <div style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#0a1020", borderRadius: "24px 24px 0 0", padding: "32px 22px 48px", border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{ color: "#64748b", fontSize: 13, lineHeight: 1.7, marginBottom: 22, textAlign: "center" }}>India's first AI Career & Finance Bodyguard<br />Free forever. Malayalam + English. 🇮🇳</div>
            ...
            <button className="btn" onClick={() => { setAccepted(true); S.set("lp_accepted", true); }} style={{ width: "100%", padding: 16, background: "linear-gradient(135deg,#06b6d4,#0891b2)", border: "none", borderRadius: 14, color: "#fff", fontWeight: 800, fontSize: 16, boxShadow: "0 4px 20px rgba(6,182,212,.3)" }}>
      ...
      {showShare && pillar && <ShareCard pillar={pillar} metrics={metrics} lastMsg={shareMsg} onClose={() => setShowShare(false)} />}
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", zIndex: 1 }}>
          ...
          <div style={{ padding: "20px 18px 0", display: "flex", justifyContent: "space-between", alignItems: "center", animation: "fadeUp .4s both" }}>
            ...
            <button className="btn" onClick={() => setShowProfile(true)} style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 100, padding: "8px 16px", color: "#94a3b8", fontSize: 12, display: "flex", alignItems: "center", gap: 7 }}>
          ...
          {(profile.name || profile.goal) && (
            <div style={{ margin: "16px 18px 0", padding: "14px 16px", background: "linear-gradient(135deg,rgba(6,182,212,.05),rgba(99,102,241,.05))", border: "1px solid rgba(6,182,212,.13)", borderRadius: 16, animation: "fadeUp .5s .1s both" }}>
              ...
              <div style={{ color: "#64748b", fontSize: 12, lineHeight: 1.7 }}>
                ...
                I am monitoring the <strong style={{ color: "#94a3b8" }}>{profile.goal || "career"}</strong> market for you. Kerala job market is active today.
              </div>
            </div>
          )}
      ...
      {screen === "chat" && pillar && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", zIndex: 1 }}>
          <div style={{ padding: "11px 14px", background: "rgba(6,11,20,.97)", borderBottom: "1px solid rgba(255,255,255,.05)", backdropFilter: "blur(24px)", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 10 }}>
            ...
            {hasMetrics && (
              <button className="btn" onClick={() => setShowShare(true)} style={{ background: `rgba(${pillar.rgb},.08)`, border: `1px solid rgba(${pillar.rgb},.2)`, borderRadius: 9, padding: "5px 10px", color: pillar.color, fontSize: 10, fontWeight: 700 }}>Share 📤</button>
            )}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 13px" }}>
            ...
            {curMsgs.length === 0 && (
              <div style={{ textAlign: "center", padding: "30px 14px", animation: "fadeUp .4s both" }}>
                ...
                <div style={{ color: "#64748b", fontSize: 12, lineHeight: 1.75, marginBottom: 20 }}>
                  {pillar.id === "cv" ? "Upload your CV for ATS analysis, or describe yourself to build a new one from scratch." : "Ask anything in Malayalam, Hindi, English, or Manglish."}
                </div>
                {profile.name && (
                ...
                {pillar.id === "cv" && (
                  <div style={{ marginBottom: 14 }}>
                    <input ref={fileRef} type="file" accept=".txt,.doc,.docx" onChange={handleFileUpload} style={{ display: "none" }} />
                    <button className="btn" onClick={() => fileRef.current?.click()} style={{ width: "100%", padding: "13px", background: `rgba(${pillar.rgb},.08)`, border: `1px solid rgba(${pillar.rgb},.25)`, borderRadius: 13, color: pillar.color, fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                      📎 Upload CV (.txt or .doc) — Get ATS Score
                    </button>
                    <div style={{ color: "#475569", fontSize: 10, textAlign: "center" }}>PDF? Copy-paste your CV text directly in the chat below</div>
                  </div>
                )}
            ...
            {curMsgs.map((msg, i) => {
              ...
              const msgMetrics = isAI ? parseMetrics(msg.content) : {};
              const hasScore = Object.keys(msgMetrics).length > 0;
              return (
                <div key={i} style={{ marginBottom: 14, display: "flex", flexDirection: "column", alignItems: isAI ? "flex-start" : "flex-end", animation: "fadeUp .3s both" }}>
                  {!isAI ? (
                    <div style={{ background: `linear-gradient(135deg,${pillar.color},${pillar.color}bb)`, borderRadius: "16px 16px 4px 16px", padding: "11px 15px", maxWidth: "82%", color: "#fff", fontSize: 13, lineHeight: 1.65, boxShadow: `0 4px 16px rgba(${pillar.rgb},.25)` }}>
                      {msg.content.length > 400 ? msg.content.substring(0, 400) + "..." : msg.content}
                    </div>
                  ) : (
                    <div style={{ background: "rgba(255,255,255,.022)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,.065)", borderRadius: "4px 16px 16px 16px", padding: "14px", maxWidth: "98%", boxShadow: "0 4px 24px rgba(0,0,0,.25)" }}>
                      ...
                      <button className="btn" onClick={() => openShare(msg.content)} style={{ marginTop: 12, padding: "7px 14px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 100, color: "#64748b", fontSize: 10, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}>
                        📤 Share this result
                      </button>
                    </div>
                  )}
          ...
          <div style={{ padding: "10px 13px 28px", background: "rgba(6,11,20,.97)", borderTop: "1px solid rgba(255,255,255,.04)", backdropFilter: "blur(24px)" }}>
            {cvFile && (
              <div style={{ marginBottom: 9, padding: "7px 13px", background: "rgba(6,182,212,.07)", border: "1px solid rgba(6,182,212,.2)", borderRadius: 10, color: "#06b6d4", fontSize: 11, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => { setCvFile(null); setInput(""); }} style={{ background: "none", border: "none", color: "#ef4444", fontSize: 13, padding: "0 4px" }}>✕</button>
              </div>
            )}
